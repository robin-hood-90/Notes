...
> [!info] Form-heavy application architecture
> This project demonstrates a production form with multiple form types: simple login, complex registration with async validation, dynamic field arrays (invoice line items), multi-step wizard, and auto-saving drafts. Uses React Hook Form, Zod validation, and RTK Query for async checks.

```mermaid
flowchart TD
    A["Form architecture"] --> B["Simple form<br/>Login<br/>React Hook Form + Zod"]
    A --> C["Complex form<br/>Registration<br/>Async validation<br/>Email exists check"]
    A --> D["Dynamic form<br/>Invoice line items<br/>useFieldArray"]
    A --> E["Multi-step<br/>Wizard<br/>Step validation<br/>Draft persistence"]
    B & C & D & E --> F["Validation library (Zod/Yup)"]
    F --> G["Schema → TypeScript types"]
    G --> H["Type-safe forms end-to-end"]
```
